...
// Footer - Comprehensive links but simplified for the assessment timeframe
    ...
    <footer className={styles.footer}>
      <div className="max-w-[1440px] mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12 mb-16">
          ...
          {[
            { title: 'Company', links: ['About Us', 'Careers', 'Blogs'] },
            { title: 'AI Solutions', links: ['Custom AI Agents', 'Voice AI', 'Support AI'] },
            { title: 'Products', links: ['JobMail AI', 'PostGenius AI'] }
          ].map((col) => (
            <div key={col.title}>
              <h4 className="font-bold mb-6">{col.title}</h4>
              <ul className="flex flex-col gap-4 text-gray-400 text-sm">
                {col.links.map(l => <li key={l}><Link href="#" className="hover:text-white">{l}</Link></li>)}
              </ul>
            </div>
          ))}
        </div>
